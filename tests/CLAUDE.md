...
| `make test-research` | varies | Falsification spike tests + cross-system research (paired with `scripts/research/`). NOT in `test-integration` (integration ignores `tests/research/` via `INTEGRATION_IGNORES`); may carry intentional `NotImplementedError` stubs during scaffold-first extraction-Bit phase per TDD-first discipline. NOT deploy-blocking. | During Phase 0 falsification work. |
