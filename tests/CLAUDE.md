...
| `make test-research` | varies | Falsification spike tests + cross-system research (paired with `scripts/research/`). NOT in `test-integration` (integration ignores `tests/research/` via `INTEGRATION_IGNORES`); may carry intentional `NotImplementedError` stubs during the scaffold-first phase of a research/falsification Bit per TDD-first discipline. NOT deploy-blocking. Phase-0 CT-MDP falsifications: F-series (F0.1 2026-05-20 SHIPPED VERDICT SURVIVE; F0.4 2026-05-21 SHIPPED VERDICT KILL; F0.5 2026-05-21 SHIPPED VERDICT SURVIVE; future F0.2/F0.3/F0.6/F0.7 entries land at their own ship). | During Phase 0 falsification work. |
