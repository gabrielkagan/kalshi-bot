...
| `make test-research` | varies | Falsification spike tests + cross-system research (paired with `scripts/research/`). NOT in `test-integration` (integration ignores `tests/research/` via `INTEGRATION_IGNORES`); may carry intentional `NotImplementedError` stubs during the scaffold-first phase of a research/falsification Bit per TDD-first discipline. NOT deploy-blocking. Current inhabitants: F0.1 (SHIPPED 2026-05-20, SURVIVE), F0.5 (scaffold 2026-05-21). | During Phase 0 falsification work. |
