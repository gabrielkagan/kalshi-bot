...
Sister cleanup atomic in same commit:
  - tests/test_executor_extraction.py::test_bot_impl_uses_l81_alias_import
  ...
  - tests/test_state_extraction.py triple-walk → quadruple-walk
    (BOT_PY + SCANNER_PY + EXECUTOR_PY + SETTLEMENT_PY).
  - tests/test_order_outcome_vocab.py SCANNED_PATHS extended with bot/settlement.py.
  ...
  - tests/test_tracker_tick_threaded.py BOT_PY → SETTLEMENT_PY retarget.
  - tests/test_logger_extraction.py + tests/test_kalshi_client_extraction.py
...
Mirrors tests/test_executor_extraction.py (Bit 9.1 path-A++) +
tests/test_scanner_extraction.py (Bit 8.1) + tests/test_state_extraction.py
(Bit 7.1 path-A++).
"""
