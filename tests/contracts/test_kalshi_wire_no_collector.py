"""D1.1.5 — ``kalshi_wire/`` must NOT import ``collector.*`` either.

Ticket 86b9zdhz2 (2026-05-16). Sister contract to ``kalshi_wire-no-bot``.
Per the D0.3 2026-05-16 AMENDMENT, ``kalshi_wire/`` is a pure-transport
leaf consumed by BOTH bot/feeds/kalshi.py AND collector/ws_connection.py.
The dependency arrow is consumer → wire, never wire → consumer:

  bot.feeds.kalshi      ─┐
                         ├─→ kalshi_wire
  collector.ws_connection ┘

A ``kalshi_wire → collector`` edge would invert the layering, couple the
wire library to the bronze-archival pipeline, and break the symmetry that
makes the advisor's "two sides of the same coin" actually work. (The
symmetric ``kalshi_wire → bot`` ban is pinned by the sister test file
``test_kalshi_wire_no_bot.py``.)

If this test fails:
- Intentional contract change: shouldn't happen — kalshi_wire is meant to
  be a leaf.
...
# ─── 1. .importlinter shape: kalshi_wire-no-collector contract ───────────────


def test_importlinter_declares_kalshi_wire_no_collector_contract():
    """``[importlinter:contract:kalshi_wire-no-collector]`` exists and is
    correctly typed."""
    cp = _read_importlinter()
    section = "importlinter:contract:kalshi_wire-no-collector"
    assert cp.has_section(section), (
        f"missing [{section}] section. D1.1.5 (86b9zdhz2) ships the "
        "structural enforcement of zero collector.* imports in kalshi_wire/."
    )
    assert cp.get(section, "type", fallback=None) == "forbidden", (
        "contract kalshi_wire-no-collector: type must be `forbidden`."
    )
    ...
    assert sources == {"kalshi_wire"}, (
        f"contract kalshi_wire-no-collector: source_modules must be exactly "
        f"{{kalshi_wire}}; got {sources}."
    )
    assert forbidden == {"collector"}, (
        f"contract kalshi_wire-no-collector: forbidden_modules must be exactly "
        f"{{collector}}; got {forbidden}."
    )


def test_orchestrator_expected_contracts_lists_kalshi_wire_no_collector():
    ...
    assert '"kalshi_wire-no-collector"' in src, (
        f"{CONTRACTS_TEST} EXPECTED_CONTRACTS tuple does NOT list "
        "`kalshi_wire-no-collector`. Add it in the same Bit that ships."
    )


# ─── 2. AST defense-in-depth — zero collector.* imports under kalshi_wire/ ──
...
def test_no_kalshi_wire_module_imports_collector():
    """AST defense-in-depth: zero ``from collector[.X] import …`` or
    ``import collector[.X]`` statements anywhere under ``kalshi_wire/``.
    """
    offenders: list[str] = []
    ...
    assert not offenders, (
        "kalshi_wire/ contains forbidden collector.* imports:\n  " +
        "\n  ".join(offenders) +
...
def test_lint_imports_fails_when_kalshi_wire_imports_collector(tmp_path: Path):
    """Negative-injection: inject ``from collector.writer import BronzeWriter``
    into a stub copy of ``kalshi_wire/ws_client.py`` and assert
    ``lint-imports`` exits non-zero with ``kalshi_wire-no-collector`` mentioned.
    """
    if not WIRE_DIR.is_dir():
    ...
    # .importlinter `root_packages` plural list; mirror the same
    # fixture-extension the D1.1.5 add made for kalshi_wire in
    # tests/contracts/test_collector_no_bot_imports.py to keep
    ...
    assert result.returncode != 0, (
        ...
        "the top of kalshi_wire/ws_client.py — the kalshi_wire-no-collector "
        "contract is not enforced.\n"
        f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )
    combined = result.stdout + result.stderr
    assert ("kalshi_wire-no-collector" in combined or "collector.writer" in combined
            or "kalshi_wire.ws_client" in combined), (
        "lint-imports failed but neither contract id `kalshi_wire-no-collector` "
