...
| `research/` | 5 (research) | `make test-research` | varies (NOT deploy-blocking) | Falsification spike tests + cross-system research (paired with `scripts/research/`). NOT in `make test` aggregate; ignored via `INTEGRATION_IGNORES` so deploy-blocking integration shards do not collect intentional `NotImplementedError` stubs during scaffold-first phase per TDD-first discipline. CT-MDP F0.1 (2026-05-20, 15 GREEN — verdict SURVIVE) + F0.4 (2026-05-21, 18 GREEN at impl ship — verdict KILL) + F0.5 (2026-05-21, 16 GREEN at impl ship — verdict SURVIVE) are the current inhabitants (49 research tests total). |
