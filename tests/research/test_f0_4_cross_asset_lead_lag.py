...
template lineage). Originally landed BEFORE implementation as **18
failing-assertion test functions covering 13 logical invariants** with
per-invariant breakdown `1=4, 2=2, 3=1, 4=1, 5=1, 6=2, 7=1, 8=1, 9=1,
10=1, 11=1, 12=1, 13=1` (Invariant 1's 4 tests are the 4 schema-pin
assertions; Invariant 2 has paired M_stale / M_responded tests;
Invariant 6 has paired kill/survive verdict tests; Invariants 12+13 are
paired full-pipeline kill/survive). All 18 tests landed RED at
scaffold-ship (impl helpers did not exist yet) and are GREEN
post-impl-Bit ship.
...
def _seed_synthetic_subthreshold_db(db_path: Path) -> None:
    """Create a minimal state.db where all cells are sub-threshold.

    Schema mirrors only the columns the script reads. BTC σ-events are
    generated by a controlled spot trajectory; laggard mids stay FLAT
    so per-event lead-edges are 0c. All 24 cells end up either
    `status="insufficient"` (cells with no events) or
    `ci_low_bonferroni == 0.0` (cells with 30+ flat-mid events) — both
    yield KILL.
    """
    ...
    base = dt.datetime(2026, 5, 15, 15, 0, 0, tzinfo=dt.timezone.utc)
    ...
    # 1. BTC spot trajectory: ~200 rows at 30s cadence (100 min, all in
    #    14-22 UTC day band). Pattern produces ~50 σ-events with varying
    #    magnitude so σ_median splits high/low cleanly.
    btc_prices = [70000.0]
    for i in range(200):
        # Baseline tiny noise (so σ ≠ 0 in the rolling window) +
        # interleaved larger returns (the σ-event triggers).
        if i % 2 == 0 and 60 <= i <= 199:
            ret = 0.01 if (i // 2) % 2 == 0 else 0.005
        else:
            ret = 0.0001 if i % 3 == 0 else -0.0001 if i % 3 == 1 else 0.0
        btc_prices.append(btc_prices[-1] * (1.0 + ret))

    eval_rows: list[tuple] = []
    laggard_base_prices = {
        "ETH": 3500.0, "SOL": 150.0, "XRP": 0.6,
        "HYPE": 30.0, "DOGE": 0.15, "BNB": 600.0,
    }
    for i in range(200):
        t = base + dt.timedelta(seconds=30 * i)
        prices = (btc_prices[i],
                  laggard_base_prices["ETH"],
                  laggard_base_prices["SOL"],
                  laggard_base_prices["XRP"],
                  laggard_base_prices["HYPE"],
                  laggard_base_prices["DOGE"],
                  laggard_base_prices["BNB"])
        eval_rows.append((t.isoformat().replace("+00:00", "Z"), *prices))
    ...
    # 2. Laggard moc: each laggard's mid is FLAT at 50/51 across the
    #    entire window. Per-event lead-edge = 0c → all cells subthreshold.
    moc_rows: list[tuple] = []
    for a in ("ETH", "SOL", "XRP", "HYPE", "DOGE", "BNB"):
        ticker = f"{prefix[a]}15M-26MAY151200-SYN"
        # 400 rows at 15s cadence — 100min span, denser than eval_opps.
        for j in range(400):
            t = base + dt.timedelta(seconds=15 * j)
            yb, ya = 50, 51
            moc_rows.append((
                ticker,
    ...
    conn.executemany(
        ...
        "no_bid_cents, no_ask_cents) VALUES (?, ?, ?, ?, ?, ?)",
        moc_rows,
    )
    conn.commit()
    conn.close()
    # Reference math import to keep the helper self-contained at module level.
    _ = math.log(1.0)
...
def _seed_synthetic_survivor_db(db_path: Path) -> None:
    ...
    # 1. BTC spot trajectory — same shape as subthreshold seed so the
    #    σ-event set is identical (only the laggard mid response differs).
    btc_prices = [70000.0]
    btc_event_ticks: list[int] = []  # eval_opps row indices where σ-event fires
    for i in range(200):
        if i % 2 == 0 and 60 <= i <= 199:
            ret = 0.01 if (i // 2) % 2 == 0 else 0.005
            btc_event_ticks.append(i)
        else:
            ret = 0.0001 if i % 3 == 0 else -0.0001 if i % 3 == 1 else 0.0
        btc_prices.append(btc_prices[-1] * (1.0 + ret))
    ...
    #    btc_event_ticks records the loop index `i` where the spot price was
    #    multiplied by (1+ret); the price change shows up between eval_opps
    #    row i and row i+1, so the event fires at base + 30s × (i+1).
    event_times = [base + dt.timedelta(seconds=30 * (i + 1)) for i in btc_event_ticks]
    moc_rows: list[tuple] = []
    for a in ("ETH", "SOL", "XRP", "HYPE", "DOGE", "BNB"):
        ticker = f"{prefix[a]}15M-26MAY151200-SYN"
        for j in range(400):
            t = base + dt.timedelta(seconds=15 * j)
            # In responded-state iff t is strictly after some σ-event
            # AND within Δt=30s of that event.
            in_response = any(
                dt.timedelta(seconds=0) < (t - ev) <= dt.timedelta(seconds=30)
