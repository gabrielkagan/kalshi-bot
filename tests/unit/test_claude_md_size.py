...
- `make test-fast` recipe invokes this file (symmetry with
  `tests/unit/test_agents_md_symlink.py` from Bit 1.3 — fast-tier pin).
"""
import os
import re
...
def test_agents_md_line_count_matches_claude_md():
    ...
    Belt-and-suspenders for `tests/unit/test_agents_md_symlink.py::
...
def test_test_unit_tier_invokes_claude_md_size_test():
    ...
    `tests/unit/test_claude_md_size.py`.

    Symmetric with
    `tests/unit/test_agents_md_symlink.py::test_test_unit_tier_invokes_agents_md_test`
    ...
    combined = "\n".join(fragments)
    _legacy = "tests/unit/test_claude_md_size.py" in combined
    _new_explicit = "tests/unit/test_claude_md_size.py" in combined
    _dir_glob = "tests/unit" in combined
    assert _legacy or _new_explicit or _dir_glob, (
        f"Unit tier (test-unit/test-fast/UNIT_FILES) doesn't invoke "
        f"tests/unit/test_claude_md_size.py (Bit 1.4). Combined surface: "
        f"{combined!r}"
    )
