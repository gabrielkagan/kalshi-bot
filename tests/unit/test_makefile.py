...
# Bit 13.3 (Sprint 13, 2026-05-11) — auto-gen agent_docs/repository_map.md.
# Walks bot/ via AST, extracts top-level classes + public functions +
# LOC per module. Complementary to tests/contracts/public_api.json
# (Pillar 1): public_api.json is the SNAPSHOT CONTRACT; repository_map.md
# is the NAVIGATION AID for agent sessions.
BIT_13_3_TARGETS = ("refresh-map",)

# Phase-0 CT-MDP falsifications (F0.1 2026-05-20, F0.4 2026-05-21, ...) —
# falsification spike tier. Paired with scripts/research/. NOT
# deploy-blocking; runs locally during falsification work. May carry
# intentional NotImplementedError stubs during the scaffold-first phase
# per CLAUDE.md TDD-first discipline.
CT_MDP_F_TARGETS = ("test-research",)

ALL_TARGETS = REQUIRED_TARGETS + OPTIONAL_TARGETS + BIT_11_3_TARGETS + BIT_11_1B_TARGETS + BIT_12_4_TARGETS + BIT_13_3_TARGETS + CT_MDP_F_TARGETS
...
def test_test_unit_invokes_invariant_tests():
    ...
    Pillar 5 renamed test-fast → test-unit. Bit 12.2 (Sprint 12,
    2026-05-11) relocated the file list into tests/unit/ and replaced
    the UNIT_FILES enumeration with a directory glob. Two valid shapes:
      (a) Legacy enumerate: trio appears inline in recipe / UNIT_FILES
      (b) Directory glob: UNIT_FILES references tests/unit/ AND the
          trio physically lives under tests/unit/
    The Bit 1.2 trio (test_pyproject.py, test_repo_hygiene.py,
    test_makefile.py) is still the load-bearing sub-second invariant.
    """
    recipe = _recipe_for("test-unit")
    assert "pytest" in recipe, "test-unit must invoke pytest."
    combined = _unit_tier_file_list()
    trio = ("test_pyproject.py", "test_repo_hygiene.py", "test_makefile.py")
    if all(fragment in combined for fragment in trio):
        return  # Shape (a) — enumerate form.
    # Shape (b) — directory glob form. Verify tests/unit is the glob and
    # the trio physically resides there.
    assert "tests/unit" in combined, (
        f"test-unit doesn't enumerate the trio inline AND doesn't "
        f"glob tests/unit. Combined surface: {combined!r}"
    )
    import os as _os
    project_root = _os.path.dirname(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))))
    unit_dir = _os.path.join(project_root, "tests", "unit")
    for fragment in trio:
        path = _os.path.join(unit_dir, fragment)
        assert _os.path.isfile(path), (
            f"test-unit globs tests/unit but {fragment!r} is missing "
            f"from {unit_dir!r}. The Bit 1.2 trio must physically live "
            f"under tests/unit/ post-Bit-12.2."
        )
...
def test_test_mutmut_invokes_mutmut_run():
    ...
    `tests/unit/test_pyproject.py::test_pyproject_mutmut_targets_engines`),
...
def test_bit_13_3_repository_map_generated():
    """The generated `agent_docs/repository_map.md` must exist (after
    running `make refresh-map` at least once) and contain the canonical
    header that flags it as auto-generated. Operators editing it
    manually defeat the purpose."""
    output = REPO_ROOT / "agent_docs/repository_map.md"
    assert output.is_file(), (
        "agent_docs/repository_map.md missing — run `make refresh-map`. "
        "Bit 13.3 ships an initial generation; the test fails until "
        "the file is committed."
    )
    content = output.read_text()
    # Canonical header — pinned so a future edit can't strip the
    # "DO NOT EDIT MANUALLY" warning without the test catching it.
