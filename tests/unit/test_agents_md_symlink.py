...
def test_test_unit_tier_invokes_agents_md_test():
    ...
    `tests/unit/test_agents_md_symlink.py`.
    ...
    assert fragments, (
        ...
        "an unrelated regression — check tests/unit/test_makefile.py."
    )
    combined = "\n".join(fragments)
    _legacy = "tests/unit/test_agents_md_symlink.py" in combined
    _new_explicit = "tests/unit/test_agents_md_symlink.py" in combined
    ...
    assert _legacy or _new_explicit or _dir_glob, (
        ...
        f"tests/unit/test_agents_md_symlink.py (Bit 1.3). Combined surface: "
        f"{combined!r}"
    )
