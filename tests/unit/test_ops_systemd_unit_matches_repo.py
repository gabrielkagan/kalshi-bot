...
def test_test_unit_tier_invokes_systemd_unit_test():
    ...
    combined = "\n".join(fragments)
    here_name = "test_ops_systemd_unit_matches_repo.py"
    _legacy = here_name in combined
    _dir_glob = "tests/unit" in combined and (REPO_ROOT / "tests" / "unit" / here_name).exists()
    assert _legacy or _dir_glob, (
        "Unit tier doesn't invoke tests/unit/test_ops_systemd_unit_matches_repo.py "
        "(Bits 1.3-1.5 + 12.2). Accept either enumerate form (file path "
        "in recipe/UNIT_FILES) or directory-glob form (UNIT_FILES = "
        "tests/unit AND file resides there). Combined surface: "
        f"{combined!r}"
    )
