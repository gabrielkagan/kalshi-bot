...
def test_test_unit_tier_invokes_scan_gate_test():
    ...
    combined = "\n".join(fragments)
    here_name = "test_post_deploy_scan_gate.py"
    ...
    assert _legacy or _dir_glob, (
        f"Unit tier doesn't invoke tests/unit/test_post_deploy_scan_gate.py "
        f"(Bit 12.2). Accept enumerate or directory-glob form. "
