...
Real caller imports (5 sites):
  ...
  - tests/integration/test_weather_ensemble_cache.py:35 (`import weather_engine`)
  - tests/integration/test_weather_ensemble_cache.py:36 (`from weather_engine import WeatherEngine, WEATHER_ENSEMBLE_CACHE_FILE`)
...
Path-literal refs:
  ...
  - tests/contracts/test_db_signatures.py:45,311 (parametrize × 2)
  - tests/contracts/test_call_sites.py:33 (ENGINE_FILES), :150 (file-open!)
  - tests/integration/test_15m_silence_alert.py:1587 (prose, cosmetic)
...
def test_no_stale_from_weather_engine_imports(repo_ast_cache):
...
def test_no_stale_import_weather_engine(repo_ast_cache):
...
_PATH_LITERAL_ALLOW = {
    ...
    "tests/integration/test_sprint_10_1b_spx_engine_move.py",      # mentions 10.1c roadmap
