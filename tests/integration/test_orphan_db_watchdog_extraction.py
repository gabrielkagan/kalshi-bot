...
Bit 9.3-ii (Sprint 9 closing, 2026-05-10):
  ...
  - bot/_impl.py: 5 function defs + 1 list def removed (~200 LOC); new
    ...
    11 `monkeypatch.setattr(bot, ...)` sites in tests/integration/test_orphan_db_watchdog.py
  ...
  - tests/integration/test_orphan_db_watchdog.py: 11 monkeypatch sites RETAIN
...
Mirrors tests/integration/test_order_flow_extraction.py (Bit 9.3.5 clean leaf shape) +
tests/integration/test_settlement_extraction.py (Bit 9.2 clean leaf with _telegram_state
...
def test_proxy_chain_resolves_detect_orphan_db_holders():
    ...
    The 11 monkeypatch sites in tests/integration/test_orphan_db_watchdog.py use
...
def test_canonical_home_resolves_all_orphan_db_names(name):
    ...
    pin reduces to "name exists on bot.orphan_db_watchdog" — which is what tests/integration/test_orphan_db_watchdog.py
...
def test_bot_impl_reexports_orphan_db_names():
    ...
    tests/integration/test_orphan_db_watchdog.py reach the orphan-DB helpers via
