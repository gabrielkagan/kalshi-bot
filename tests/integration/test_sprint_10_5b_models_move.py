...
def test_no_stale_from_models_imports(repo_ast_cache):
...
def test_no_stale_import_models(repo_ast_cache):
...
def test_scanner_extraction_regex_pin_uses_new_path():
    """tests/integration/test_scanner_extraction.py:1058 has a regex pin checking the
    ...
    src = (REPO_ROOT / "tests" / "integration" / "test_scanner_extraction.py").read_text()
    assert r"from bot\.models import" in src, (
        "tests/integration/test_scanner_extraction.py: the regex pin checking the "
