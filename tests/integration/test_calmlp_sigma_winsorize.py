...
CAL_MLP_DIR = Path(__file__).resolve().parents[2] / 'scripts' / 'cal_mlp'
...
def test_should_block_tm96_winsorizes_sigma():
    ...
    import sys
    from pathlib import Path
    cal_mlp = Path(__file__).resolve().parents[2] / 'scripts' / 'cal_mlp'
...
def test_post_hoc_processor_winsorizes_sigma_when_loading_db_value():
    ...
    import ast
    from pathlib import Path
    src_path = Path(__file__).resolve().parents[2] / 'scripts' / 'cal_mlp' / 'post_hoc_processor.py'
...
def test_extract_uses_features_module_attr_not_import_copy():
    ...
    """
    extract_path = (
        Path(__file__).resolve().parents[2] / 'scripts' / 'cal_mlp' / 'extract_data.py'
    )
