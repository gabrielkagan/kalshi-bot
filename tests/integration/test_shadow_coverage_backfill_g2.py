...
class TestG2BackfillIntegrationWithMockedAPI:
    ...
    def test_backfill_writes_xasset_spots(self, tmp_path):
        ...
        def _mock_fetch(asset, start_iso, end_iso):
            ...
            base = {"BTC": 67432.5, "ETH": 3210.5,
                    ...
                    "ADA": 0.4512, "BCH": 512.3, "NEAR": 2.2427, "ZEC": 38.15}[asset]
    ...
    def test_backfill_skips_already_populated(self, tmp_path):
        from shadow_coverage_backfill import backfill_xasset_spots
        sm = _make_db_with_eval_schema(tmp_path)
        # Row that ALREADY has all 11 cross-asset spots populated (live
        # row, post NEAR/ZEC T1 2026-09-05). Bit 2 (2026-05-11) widened
        ...
        # NEAR/ZEC T1 (2026-09-05, 86bbvdc8y) extended it to 11.
        # Post-extension, a "fully populated" row MUST include the
        # ada/bch/near/zec columns — without them, the widened WHERE predicate would pick
