...
Sister cleanup atomic in same commit:
  ...
  - tests/integration/test_state_extraction.py quadruple-walk → quintuple-walk
    (BOT_PY + SCANNER_PY + EXECUTOR_PY + SETTLEMENT_PY + MAIN_LOOP_PY).
  - tests/integration/test_settlement_extraction.py + tests/integration/test_executor_extraction.py
  ...
  - tests/contracts/test_order_outcome_vocab.py SCANNED_PATHS extended.
  ...
  - tests/integration/test_bot_h2_step2_wiring.py concat list extended.

Mirrors tests/integration/test_settlement_extraction.py (Bit 9.2) + tests/integration/test_executor_extraction.py
(Bit 9.1) + tests/integration/test_scanner_extraction.py (Bit 8.1) + tests/integration/test_state_extraction.py
(Bit 7.1).
...
def test_no_main_loop_no_impl_toplevel_contract_added():
    ...
    # Sanity: 10 contracts post-D2.1 (2026-05-17). The 5 bot-side contracts
    ...
    # ticket 86b9ypn49) plus D1.1.5's two contracts (ticket 86b9zdhz2):
...
def test_main_loop_instantiable_via_new_without_init():
    ...
    tests/integration/test_stale_ticker_cleanup.py + tests/integration/test_cache_staleness_watchdog.py."""
    import bot
    import bot.main_loop  # noqa: F401 (Bit 9.3-iii.c — explicit submodule import; bot.main_loop.X access)
