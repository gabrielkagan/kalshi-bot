...
def _read_bot_py():
    """Cached read of bot/_impl.py + bot/state.py + bot/scanner/__init__.py
    concat.

    Bit 7.1 (2026-05-09): StateManager moved to bot/state.py —
    `insert_evaluated_opportunity` and other StateManager methods that the
    AST guards below scan for now live there.

    Bit 8.1 (2026-05-10): OpportunityScanner moved to
    bot/scanner/__init__.py — Edit 4 hook (`_calmlp_annotate_async`),
    `if _pt == "15m":` gate, queue-snapshot `_shadow_diag.copy()` sites,
    and the cal_mlp_* prefix-strip comprehensions all moved with the class.

    The concat lets every existing AST guard find its target without
    per-test retargeting.
    """
    p = Path(__file__).resolve().parents[2] / 'bot/_impl.py'
    if not p.exists():
        pytest.skip('bot/_impl.py not present (running on rebuild branch without bot/_impl.py edits)')
    src = p.read_text()
    state_p = Path(__file__).resolve().parents[2] / 'bot/state.py'
    if state_p.exists():
        src = src + '\n' + state_p.read_text()
    scanner_p = Path(__file__).resolve().parents[2] / 'bot' / 'scanner' / '__init__.py'
    if scanner_p.exists():
        src = src + '\n' + scanner_p.read_text()
    return src
...
def test_thread_env_imported_before_numerical_libs_in_bot_impl():
    ...
    bot_py = Path(__file__).resolve().parents[2] / 'bot/_impl.py'
...
def test_thread_env_imported_before_numerical_libs_in_bot_boot():
    ...
    **Form**: `__import__()` runtime call rather than `import bot._thread_env`
    ...
    """
    import ast
    boot_py = Path(__file__).resolve().parents[2] / 'bot/boot.py'
