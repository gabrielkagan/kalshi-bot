...
class TestHourlyConstants:
    ...
    def test_kill_switch_default_off(self):
        ...
        comparisons in sister tests (sister extraction tests hold references to constants).
        We check the env-var default behavior directly.
        """
        env = os.environ.pop("HOURLY_LIVE_ENABLED", None)
        try:
            # The constant is `os.environ.get("HOURLY_LIVE_ENABLED", "0") == "1"`.
            actual_live = os.environ.get("HOURLY_LIVE_ENABLED", "0") == "1"
            assert actual_live is False
            # HOURLY_OBSERVATION_ONLY is `not HOURLY_LIVE_ENABLED` per Bit 3.1 derivation.
            actual_obs = not actual_live
            assert actual_obs is True
        finally:
            if env is not None:
    ...
    def test_hourly_excluded_assets(self):
        ...
        assert "ETH" not in bot.constants.HOURLY_EXCLUDED_ASSETS
...
class TestMarketConfigSync:
    ...
    def test_hourly_config_values(self):
        ...
        assert cfg.excluded_assets == frozenset({"SOL", "XRP", "HYPE", "DOGE", "BNB"})
    ...
    def test_15m_config_unchanged(self):
        ...
        assert cfg.max_entry_price == bot.constants.MAX_ENTRY_PRICE  # 99, not 59
...
class TestSizingIsolation:
    ...
    def test_hourly_bankroll_fraction_value(self):
        ...
        assert not hasattr(bot.constants, "CRYPTO_BANKROLL_FRACTION")
...
class TestEdgeCapIsolation:
    ...
    def test_15m_has_no_max_edge(self):
        ...
        assert hasattr(bot.constants, "MIN_EDGE_BY_PRICE")
        # No 15M-specific max edge constant
        assert not hasattr(bot.constants, "MAX_EDGE")
    ...
    def test_15m_fee_uses_single_contract(self):
        ...
        assert not hasattr(bot.constants, "CRYPTO_MAX_EDGE")
...
class TestHourlyDC:
    ...
    def test_hourly_dc_constants(self):
        ...
        assert bot.constants.HOURLY_DC_ASSUMED_PROB == 0.97
        assert bot.constants.HOURLY_DC_CONTRACTS == 25
        assert bot.constants.HOURLY_DC_MIN_SIGMA == 0.000250
        assert bot.constants.HOURLY_DC_ASSETS == {"BTC"}
        assert bot.constants.HOURLY_DC_MAX_PER_WINDOW == 1
    ...
    def test_hourly_dc_independent_of_sub60c_killswitch(self):
        ...
        assert bot.constants.HOURLY_DC_ENABLED is True or bot.constants.HOURLY_DC_ENABLED is False
...
class TestObservationGate:
    ...
    def test_15m_observation_mode_independent(self):
        ...
        assert "HOURLY_LIVE_ENABLED" not in str(bot.constants.OBSERVATION_MODE)
