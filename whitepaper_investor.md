...
\item Adjacent verticals — each follows the same shadow→validate→promote pipeline:
  \begin{itemize}
  \item \textbf{Weather temperature} --- {{WEATHER_CITY_COUNT}} US cities; NO-side LIVE in 1-contract verification mode since Apr 11, YES-side observation
  \item \textbf{S\&P 500 intraday} --- equity-adapted volatility model with VIX integration; observation only
  \item \textbf{Live sports} --- {{SPORTS_LEAGUE_COUNT}} leagues including NFL, NBA, EPL, ATP/WTA Tennis; observation only
