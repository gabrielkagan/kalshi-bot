...
import bot.notifier as _telegram_state  # Bit 8.1 path-A++ (2026-05-10): alias for `_telegram_state._TELEGRAM` module-attribute access. The singleton lives in bot/notifier.py alongside TelegramNotifier; the module-attribute access pattern preserves mutation freshness across all four consumers post-Bit-9.2 (this module for MainLoop reads + bot/scanner/__init__.py + bot/executor.py + bot/settlement.py — verify counts with `grep -c '_telegram_state\._TELEGRAM' bot/_impl.py bot/scanner/__init__.py bot/executor.py bot/settlement.py` — every reader through this alias sees writes immediately because we go through the module reference, NOT a captured-by-value binding). Mirrors the Bit 6.3 path-B `_cal_state` pattern. MainLoop.__init__ writes via `_telegram_state._TELEGRAM = self.telegram` (drops the previous `global _TELEGRAM` declaration). NOTE: explicit `import bot.notifier as ...` (NOT `from bot import notifier as ...`) — the latter form goes through `_BotProxy.__getattr__` and triggers a partial-module ImportError of bot._impl from inside bot.scanner during its load.
...
# post-Bit-9.2 with `grep -c '_telegram_state._TELEGRAM' bot/_impl.py
# bot/scanner/__init__.py bot/executor.py bot/settlement.py`; mirrors
# the `_cal_state._CALIBRATION_ENGINE` pattern; 4 consumers post-Bit-9.2).
# The plain `from bot.notifier import _TELEGRAM` form would NOT propagate
# runtime mutation because `from`-imports capture by value at import time.
