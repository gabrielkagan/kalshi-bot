...
PEP 562 `__getattr__` alone is insufficient: many external WRITE sites
(`unittest.mock.patch("bot.X", ...)` in test_execution.py + test_ladder_escalation.py;
`patch.object(bot, X, ...)` in test_15m_silence_alert.py + others;
`monkeypatch.setattr(bot, X, ...)`; direct `bot.X = value`) plus 3
production kill-switch self-mutation sites in `bot/_impl.py`
(lines 11578/11598/11618). A bare `__getattr__` makes
...
class _BotProxy(types.ModuleType):
    ...
    def __setattr__(self, name, value):
        ...
        # forgets the direct form, AND for the many `mock.patch("bot.X")` /
        # `patch.object(bot, "X")` test-suite sites that target the package
        # directly.
