...
are DATA artifacts, not source — they stay at repo root post-Bit-12.1.

Data-file paths below (DIST_CONFIG_PATH, EGARCH_STATE_PATH,
EGARCH_BLEND_STATE_PATH) are anchored to repo root via __file__ to survive
...
# Anchored via _REPO_ROOT (Bit 12.1) — pre-move bare "dist_config.json" was
# CWD-relative and silently fell back to defaults under non-repo-root CWD.
DIST_CONFIG_PATH = os.path.join(_REPO_ROOT, "dist_config.json")
