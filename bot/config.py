"""Shared runtime constants for kalshi-bot (probability/EGARCH/position-sizing).

Sprint 12 Bit 12.1 (2026-05-12) relocated `config.py` from repo root to
`bot/config.py`. The companion module `bot/constants.py` (Bit 3.1) is the
canonical home for UPPER_SNAKE constants laundered into the bot package via
explicit `from bot.constants import ...`; this file (`bot.config`) is the
runtime-tunable peer that contains derived values, NIG/Student-t distribution
state, EGARCH MLE bounds, and Kelly sizing tiers. The split is historical
(extracted from the original repo-root bot.py / later bot/_impl.py during
Sprint 4) — `bot.config` and `bot.constants` are intentionally NOT merged;
they have different volatility profiles (sizing tiers change rarely; market
config changes per asset).

Consumers reach values via explicit `from bot.config import ...` (engines,
feeds, fetchers, scanner, models, main_loop) or via `bot.runtime_config`
which PEP-562 dual-probes `bot.constants → bot.config` on each access
(used by dashboard_snapshot + supabase_sync for kill-switch freshness).

`config.json` (auto-generated by scripts/ops/extract_config.py) and
`dist_config.json` (calibration output written by scripts/ops/calibrate_dist.py)
are DATA artifacts, not source. `dist_config.json` lives at
`ops/runtime/dist_config.json` post Sprint 14-A Bit 3 (2026-05-17, ticket
86b9zfbt8); `config.json` is still at repo root pending Bit 4.

Data-file paths below (DIST_CONFIG_PATH, EGARCH_STATE_PATH,
EGARCH_BLEND_STATE_PATH) are anchored via __file__ to survive
CWD changes (tests, mac dry-runs, ad-hoc scripts). Pre-Bit-12.1 these were
CWD-relative bare strings — accidentally-correct in production where systemd
starts the bot from repo root, but silent-fall-back-to-defaults everywhere
else.
"""
...
# bot/config.py lives 2 directories deep under the repo root
# (<repo>/bot/config.py). Mirrors the bot/engines/weather_engine.py:806-807
# precedent for non-leaf bot/ modules that anchor repo-root data files.
_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))

# ─── Assets ──────────────────────────────────────────────────────────────────
ASSETS = ["BTC", "ETH", "SOL", "XRP", "HYPE", "DOGE", "BNB", "ADA", "BCH", "NEAR", "ZEC"]  # ADA/BCH = 15M shadow (T1 2026-05-30); NEAR/ZEC = 15M shadow (T1 2026-09-05, 86bbvdc8y); shadow-gated via <ASSET>_15M_SHADOW
...
# Anchored via _REPO_ROOT (Bit 12.1) — pre-Bit-12.1 bare "dist_config.json" was
# CWD-relative and silently fell back to defaults under non-repo-root CWD.
# Sprint 14-A Bit 3 (2026-05-17, ticket 86b9zfbt8) relocated the data artifact
# from repo root → ops/runtime/ as part of the root-cleanup track. Pinned by
# tests/contracts/test_bit_12_1_config_consolidation.py::
# test_dist_config_path_anchored_to_ops_runtime.
DIST_CONFIG_PATH = os.path.join(_REPO_ROOT, "ops", "runtime", "dist_config.json")
...
EGARCH_STATE_PATH = os.path.join(_REPO_ROOT, "egarch_state.json")  # Bit 12.1 __file__-anchored
...
EGARCH_BLEND_STATE_PATH = os.path.join(_REPO_ROOT, "egarch_blend_state.json")  # Bit 12.1 __file__-anchored
