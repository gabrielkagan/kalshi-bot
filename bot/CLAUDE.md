...
**Bit 9.2 (path-A++, 2026-05-10)**: `SettlementTracker` lives in `bot/settlement.py` — re-imported via `from bot.settlement import SettlementTracker, discover_active_windows` (line ~117 of bot/_impl.py). The module-level `discover_active_windows()` function ships with SettlementTracker per master plan Phase Z+AA bundle decision (settlement-adjacent in source layout, sole runtime caller is MainLoop._refresh_active_windows; deviates from original Phase AA placement with MainLoop in Bit 9.3 per USER decision via AskUserQuestion). The Bit 9.1 L81 alias-import `from bot.helpers.raw_api_journal import append_raw_api_journal as _append_raw_api_journal` RETIRED atomically (zero callers remain in bot/_impl.py — both historical SettlementTracker callers moved to bot/settlement.py with the public name `append_raw_api_journal`, mirroring bot/executor.py:98). The `_telegram_state._TELEGRAM` consumer enumeration extends to 4 modules: bot/_impl.py for MainLoop reads + bot/scanner/__init__.py + bot/executor.py + **bot/settlement.py** (verify with `grep -c '_telegram_state._TELEGRAM' bot/_impl.py bot/scanner/__init__.py bot/executor.py bot/settlement.py`). No new `.importlinter` carve-out needed — SettlementTracker has zero references to names defined below the line-117 re-export point in bot/_impl.py; clean leaf extraction; net contracts stays at 5. Bundled bug fix (ticket 86b9vppn3): pre-existing UnboundLocalError 'best_ask' in OpportunityScanner.scan() low_probability_15m insert_rejection branch (predates Bit 8.1 per git blame) — initialize best_ask=None at iteration start. Sprint 9 ⅔ done after Bit 9.2 ships here; Bit 9.3 (MainLoop → bot/main_loop.py) is the last documented Sprint 9 leaf. Per master plan, Bit 9.3-ii ultimately deletes bot/_impl.py; the residual OrderFlowEngine (~122 LOC) + KalshiOrderFlowTracker (~243 LOC) classes still live in bot/_impl.py post-Bit-9.2 and need to be relocated either as part of Bit 9.3 or as a Sprint 9 / Sprint 10 follow-up — surface this scope question when planning Bit 9.3.

**Bit 8.1 (path-A++, 2026-05-10) — post-Bit-9.2 state**: `OpportunityScanner` lives in `bot/scanner/__init__.py` — re-imported via `from bot.scanner import OpportunityScanner`. The `_get_order_executor()` single-name late-binding helper that originally lived inside `bot/scanner/__init__.py` for the 34 `OrderExecutor.X(...)` static-method call sites was RETIRED in Sprint 9 Bit 9.1; scanner now uses top-level `from bot.executor import OrderExecutor` directly. The `scanner-no-impl-toplevel` `.importlinter` contract was dropped in the same atomic Bit 9.1 commit (net contracts: 6 → 5). The Bit 8.1 `_TELEGRAM` singleton relocation to `bot/notifier.py` (path-A++) stands; the singleton is now reached from FOUR consumer modules post-Bit-9.2 — `bot/_impl.py` (MainLoop reads only), `bot/scanner/__init__.py`, `bot/executor.py`, `bot/settlement.py` — via `import bot.notifier as _telegram_state` plus `_telegram_state._TELEGRAM` module-attribute access (mutation freshness preserved across all four). The 13 consumer call sites (12 OrderExecutor static-method calls + 1 MainLoop static-method call referencing `OpportunityScanner._best_yes_ask_cents` / `_convert_orderbook_fp`) + ~30 test-suite instantiations (`bot.OpportunityScanner(...)` via `_BotProxy` → bot._impl.OpportunityScanner → bot.scanner.OpportunityScanner) all resolve via the line-115 re-export. **Path-A++ relocation of `_TELEGRAM`**: the module-level singleton moved from `bot/_impl.py` to `bot/notifier.py` alongside the `TelegramNotifier` class. All four of `bot/_impl.py`, `bot/scanner/__init__.py`, `bot/executor.py`, and `bot/settlement.py` reach it via `import bot.notifier as _telegram_state` plus `_telegram_state._TELEGRAM` module-attribute access — preserves mutation freshness across all four consumers (mirrors Bit 6.3 path-B `_cal_state._CALIBRATION_ENGINE` pattern). `MainLoop.__init__` writes via `_telegram_state._TELEGRAM = self.telegram` (drops the previous `global _TELEGRAM` declaration). **Path-A++ scanner→executor coupling (post-Bit-9.1 state)**: the 34 `OrderExecutor.X(...)` static-method call sites in `scan()` originally used a `_get_order_executor()` single-name late-binding helper inside `bot/scanner/__init__.py` (Bit 8.1) — that helper was RETIRED in Sprint 9 Bit 9.1 atomically with the OrderExecutor extraction. Scanner now uses top-level `from bot.executor import OrderExecutor` directly. **`.importlinter` (post-Bit-9.1)**: the `scanner-no-impl-toplevel` contract was added in Bit 8.1 (parallel to `state-no-impl-toplevel` from Bit 7.1 fu1) and DROPPED in Sprint 9 Bit 9.1 atomically with the helper retirement (net contracts: 6 → 5). Sister Bit 8.2 (`bot/scanner/CLAUDE.md`) shipped 2026-05-10; Bit 8.3 internal scanner split (`discover.py`/`evaluate.py`/`gates.py`/`shadow.py`) deferred 7d post-Bit-8.1 per master plan. Sprint 8 closed at Bit 8.2.
