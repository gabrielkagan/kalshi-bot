...
- `scripts/backfill/crypto_replay_backfill.py` — HYPE/DOGE/BNB Phase 2 replay backfill (renamed from `hype_doge_replay_backfill.py` in Bit F, 2026-05-21, ticket `86ba1wpck`; original ship 86b9wy7v3, 2026-05-12); per-market `replay_market()` calls `compute_hour_sin_cos` + `compute_derived_features` + `apply_sigma_winsor` for the lock-step features written to `historical_replay_calmlp`. Bit F widened `ASSETS` from `("HYPE","DOGE")` → `("HYPE","DOGE","BNB")` and added the `spot_staleness_seconds` audit column.
...
Canonical 7-set (`'candidate'` baseline + 4 pre-B1 bleed stages + 2 B1
stages) lives in `bot.helpers.cohort_attribution.COHORT_PARTITION_STAGES`
(Money Printer Roadmap P1.1, ticket `86b9x3kgd`, 2026-05-12; extended
by B1 to 7-set 2026-05-21) — extend that constant when adding a new
cell-block stage and the cohort aggregator + dashboard + alerts +
weekly report pick it up automatically.
