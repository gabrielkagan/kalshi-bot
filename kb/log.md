...
Low-Price Near-Expiry (LPNE): intercepts BTC at 80-87c when STC<=120s at the price floor check. Data: 97.6% WR on 42 signals (p=0.031), near-expiry advantage p=0.006. Fixed 50-contract sizing (LOW_STC_SIZING_CAP halves most to 25). Probability gate: `cal_prob >= price/100` (model must believe break-even — LPNE intercepts pre-cascade, so `cal_prob` is the bound name; the original `final_prob` claim was retracted at 3 sites Apr 6 (`f25cfe96`) + 4th site B3 2026-05-18). Separate _execute_lpne_taker executor. NBBO gate lowered from 86c to 80c for BTC. BTC_MIN_ENTRY_PRICE unchanged at 88c — LPNE intercepts before floor rejection.
...
## [2026-05-18] fix | B3 — LPNE insert_evaluated_opportunity used unbound `final_prob` (4th site missed by Apr 6 fix)
The April 6 fix (`f25cfe96`) patched 3 LPNE sites — the cal-gate at the if-guard, the LPNE_CANDIDATE log, and the candidate dict — all swapped `final_prob` → `cal_prob`. The 4th site, the `self._state.insert_evaluated_opportunity(..., calibrated_prob=final_prob, ...)` call inside the same LPNE block, was missed. Its UnboundLocalError was caught by the surrounding `except Exception:` and silently dropped every LPNE `evaluated_opportunities` row (training/audit data invisible). B3 (ticket `86b9zud6t`) swaps the kwarg to `cal_prob` (matching the rest of the LPNE block). Sister doc `kb/strategies/lpne.md` retracted the stale `final_prob >= best_ask / 100.0` claim in the same commit. Regression test: `tests/integration/test_lpne_final_prob_unbound_regression.py` (3 AST guards — call-site exists, kwarg is `cal_prob`, no `final_prob` token in the LPNE block).
