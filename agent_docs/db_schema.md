...
| tape_rv300 | REAL | (Bit V.3, 2026-06-12) Decision-time **independent** trailing-300s tape realized vol — `bot.helpers.tape_rv.trailing_rv300` over the CoinbaseFeed 1s buffer, EXACT parity with the validated longshot/twaplock backtests' `rv_5s` (L-VOL-1). Auto-filled on every Coinbase scan-path insert from `StateManager._scan_tape_rv_cache[asset]` (the V.1 cache; honest-NULL pop — the scanner pops the slot whenever rv300 is None: warmup, buffer-shape gap, or Bit-S.1 event-time spot staleness > 30s). NULL = honest abstention, never a fabricated number. The soak denominator: the pre-registered re-arm gate (kb/decisions/longshot-twap-live-small-plan.md, 2026-06-12) reads per-asset median `raw_blended_rv / tape_rv300` ∈ [0.8, 1.25] via the `/live-small` soak section. UPSERT-stable via `COALESCE` (first/decision-time reading survives). Pinned by `tests/contracts/test_vol_honesty_instrumentation.py`. Postmortem: `kb/failures/vol-engine-beta-dvol-deflation-jun12.md` (L-VOL-2). |
| raw_blended_rv | REAL | (Bit V.3, 2026-06-12) The RAW engine `blended_rv` BEFORE the V.1 `max(blended_rv, rv300)` selection — V.1-R1-M2: the rows' `volatility` column carries the max on live-small rows, so a deflation ratio computed off it would be ≥ 1 always and could never detect deflation; THIS column carries the engine's unmodified estimate. Auto-filled from `StateManager._scan_raw_blended_rv_cache[asset]`, **GATED on `_scan_tape_rv_cache` holding a value for the asset** — the raw cache is overwrite-only (never popped, V.1-R5), so tape-present is the freshness certificate that keeps a stale raw estimate from leaking onto rows whose tape side honestly abstained; the only consumer (the honesty ratio) is undefined without the denominator anyway. Pairing invariant on auto-filled rows: `raw_blended_rv` non-NULL ⇒ `tape_rv300` non-NULL. UPSERT-stable via `COALESCE`. Same Bit ships the ONLINE twin: `bot/helpers/vol_honesty.py::VolHonestyMonitor` (scanner-fed, 60s/asset check, WARN `VOL_HONESTY_BREACH` + Telegram 1/hour/asset outside [`VOL_HONESTY_LOW`=0.6, `VOL_HONESTY_HIGH`=1.8]). |
