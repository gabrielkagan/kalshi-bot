...
| rti_synthetic | REAL | (Ticket 86ba64h2w, B2b-1, 2026-05-28; cid=142) **SHADOW-ONLY.** Decision-time multi-venue synthetic CFB-shape Real-Time-Index reconstructed in-bot from 4-venue L2 books (Coinbase/Kraken/Bitstamp/Gemini) by `bot/feeds/synthetic_rti_feed.py::SyntheticRTIFeed` (which wraps the validated `bot.feeds.synthetic_rti.compute_synthetic_rti` aggregator). Staged once per scan tick into `StateManager._scan_rti_cache[asset]` from `SyntheticRTIFeed.get_cached_synthetic(asset)` (an O(1) read of the feed's off-hot-path sampler cache; the ~7ms/asset compute runs on the sampler daemon, NOT in `scanner.scan()` — keeps the SCAN_BODY_SLOW budget intact) and auto-filled across the Coinbase scan-path insert sites — mirrors the `_scan_cx_gap_cache` / `spot_staleness_seconds` precedent. **Read by a decision path (scanner `_effective_decision_spot`) ONLY for assets in `SYNTHETIC_RTI_LIVE_ASSETS`** (RTI-6 per-asset go-live gate; default EMPTY ⇒ the zero-live-decision-change invariant holds for every asset). Pinned by `tests/contracts/test_synthetic_rti_shadow_invariant.py` + `tests/contracts/test_rti_live_per_asset.py`. Paired with the live single-venue Coinbase `spot_price` + `market_price` + settled outcome already on the row → the corpus Bit 3 retrains on. NOTE: for a promoted asset (in `SYNTHETIC_RTI_LIVE_ASSETS`), `spot_price` still logs the Coinbase spot while the row's prob / `z_score` are computed on the RTI spot — the two intentionally differ; `rti_synthetic` carries the decision basis. NULL when the kill-switch `SYNTHETIC_RTI_ENABLED` is OFF (the default), on non-Coinbase scan paths (SPX/weather/sports), when the sampler cache is stale (WS outage), or for backfill/test callers. UPSERT-stable via `COALESCE`. See `kb/decisions/b2b-1-core-shadow-plan.md` + `kb/decisions/b2b-multi-venue-signal-program-plan.md`. |
...
| tape_rv300 | REAL | (Bit V.3, 2026-06-12; V.3-R1-M1 atomic-pair fix round) Decision-time **independent** trailing-300s tape realized vol — `bot.helpers.tape_rv.trailing_rv300` over the CoinbaseFeed 1s buffer, EXACT parity with the validated longshot/twaplock backtests' `rv_5s` (L-VOL-1). Auto-filled ATOMICALLY with `raw_blended_rv` from the single `StateManager._scan_vol_pair_cache[asset] = (raw_blended_rv, tape_rv300)` — written/popped at the scanner's 15M `_strategy_vol` seam so both halves are same-tick by construction; popped on every rv300-None vol pass (warmup, buffer-shape gap, Bit-S.1 event-time spot staleness > 30s) and in the 15M loss-cooldown branch (which skips the seam for ≤2h); fill gated to `product_type in (None, '15m')` — hourly/SPX/weather rows are both-NULL by construction (unlike `spot_staleness_seconds`, hourly does NOT carry this pair: the raw half is only seamed on 15M passes). NULL = honest abstention, never a fabricated number. The soak denominator: the pre-registered re-arm gate (kb/decisions/longshot-twap-live-small-plan.md, 2026-06-12) reads per-asset median `raw_blended_rv / tape_rv300` ∈ [0.8, 1.25] via the `/live-small` soak section (query F additionally excludes `filter_stage LIKE 'silent_%'` as defense-in-depth). UPSERT-stable via `COALESCE` (first/decision-time reading survives). Pinned by `tests/contracts/test_vol_honesty_instrumentation.py` + `tests/integration/test_tape_rv_parity.py::TestStaleVolPairLeakRegression`. Postmortem: `kb/failures/vol-engine-beta-dvol-deflation-jun12.md` (L-VOL-2). |
| raw_blended_rv | REAL | (Bit V.3, 2026-06-12; V.3-R1-M1 atomic-pair fix round) The RAW engine `blended_rv` BEFORE the V.1 `max(blended_rv, rv300)` selection — V.1-R1-M2: the rows' `volatility` column carries the max on live-small rows, so a deflation ratio computed off it would be ≥ 1 always and could never detect deflation; THIS column carries the engine's unmodified estimate. Auto-filled ATOMICALLY with `tape_rv300` from the single pair cache above — PERSIST-BOTH-OR-NEITHER: the cache only ever holds complete same-tick pairs, the fill fires only when the caller supplied neither kwarg, and one explicit side never pairs with a cached other. (The pre-fix split caches — overwrite-only raw gated on tape-present — let a frozen raw ride a fresh tape onto hourly rows and a frozen pre-cooldown pair onto `silent_loss_cooldown` trace rows.) Pairing invariant on auto-filled rows: `raw_blended_rv` non-NULL ⇔ `tape_rv300` non-NULL. UPSERT-stable via `COALESCE`. Same Bit ships the ONLINE twin: `bot/helpers/vol_honesty.py::VolHonestyMonitor` (scanner-fed from the seam LOCALS — same-tick by definition, 60s/asset check, WARN `VOL_HONESTY_BREACH` + Telegram 1/hour/asset outside [`VOL_HONESTY_LOW`=0.6, `VOL_HONESTY_HIGH`=1.8]). |
...
- **historical_replay_calmlp** — Phase 2 HYPE/DOGE/BNB replay-backfill corpus (originally HYPE/DOGE ticket `86b9wy7v3`, 2026-05-12; BNB onboarded via Bit F ticket `86ba1wpck`, 2026-05-21; Mac-local SQLite — operator-supplied via `scripts/backfill/crypto_replay_backfill.py --db PATH`. DO NOT point at production `state.db` to avoid contaminating audit queries — `*.db` is gitignored via top-level pattern). 21 cols (P2.3.b-fu2 2026-05-13 ticket `86b9xtam7` added `threshold REAL` to preserve sub-cent strike precision; legacy `strike_cents INTEGER` retained for back-compat with pre-fix HYPE corpus; Bit F 2026-05-21 added `spot_staleness_seconds REAL` audit column): `ticker TEXT, evaluation_time TEXT, asset TEXT CHECK IN ('HYPE','DOGE','BNB'), strike_cents INTEGER, threshold REAL, close_time TEXT, open_time TEXT, raw_prob REAL, calibrated_prob REAL, blended_prob REAL, spot_at_evaluation REAL, sigma_at_evaluation REAL, hour_sin REAL, hour_cos REAL, prob_breakeven_gap REAL, sigma_winsorize REAL, spot_staleness_seconds REAL, result TEXT CHECK IN ('yes','no'), settlement_value INTEGER, data_provenance TEXT, replay_run_ts INTEGER, PRIMARY KEY (ticker, evaluation_time)`. PK enables idempotent `INSERT OR REPLACE`. `ensure_schema()` migrates pre-fu2 DBs in place via `ALTER TABLE ADD COLUMN threshold REAL` + `ADD COLUMN spot_staleness_seconds REAL` when columns are absent — but CANNOT widen the asset CHECK in place (SQLite ALTER limitation). The CHECK widening (HYPE/DOGE → HYPE/DOGE/BNB) is operator-driven via the standalone `scripts/ops/migrate_replay_table_bit_f.py` (CREATE-COPY-DROP-RENAME pattern; idempotent — detects post-Bit-F state and no-ops). `data_provenance='replay_phase2_v1'` stamps every row. Phase 2 v1 honest-NULLs: `blended_prob` (no HYPE/DOGE/BNB trained cal_mlp predictor exists in PRODUCTION's `_calmlp_predictors` cache at runtime — it constructs only BTC/ETH/SOL/XRP; Bit F trained Mac-local BNB artifacts — extract bundle at `data/cal_mlp/BNB/<train_id>/` + trained weights + phase5 conformal at `models/cal_mlp_BNB/<train_id>/` with a local `models/cal_mlp_BNB/CURRENT` pointer — but `models/` is gitignored and the VPS production CURRENT pointer was NOT flipped, so it is NOT live-deployed; cross-asset transfer eval is fu1 of 86b9wy7v3), `prob_breakeven_gap` (no historical Kalshi orderbook), `spot_staleness_seconds` (NULL when warmup is empty — same NULL invariant as `spot_at_evaluation`; pre-Bit-F HYPE/DOGE rows also have NULL since the column was added 2026-05-21 after their backfill). Pre-fu2 HYPE rows have NULL `threshold` (`extract_data_replay.build_feature_frame` falls back to `strike_cents/100.0` per `kb/findings/replay-backfill-strike-precision-bug-may13.md`). Expected ~15,700 rows post-Bit-F (~4,897 HYPE + ~4,897 DOGE + ~5,906 BNB; HYPE/DOGE per Phase 1 corpus survey — `kb/findings/hype-doge-kalshi-market-history-may12.md`; BNB spans 76 days 2026-03-07 → 2026-05-21 with 16 floor_strike=None skips per the Apr-13 incident parity). Writer: `scripts/backfill/crypto_replay_backfill.py::replay_market()`. Consumers: sister `86b9wy15n` calibration health check + Bit H 3-asset head-to-head verdict.
