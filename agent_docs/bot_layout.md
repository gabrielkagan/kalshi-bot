...
bot/_impl.py is **26,117 lines** as of 2026-05-08 (post-Bit-4.2 TelegramNotifier extraction; Bit 3.1 moved ~1,200 lines of constants to `bot/constants.py`, Bit 3.2 moved ~790 lines of helpers to `bot/helpers/*`, Bit 4.1 moved 65 lines of Logger to `bot/logger.py`, Bit 4.2 moved 30 lines of TelegramNotifier to `bot/notifier.py`). Class line ranges below
...
| Lines | Section |
|---|---|
| 1–~110 | Header import block (`bot._thread_env` imports BEFORE `numpy` — load-bearing per CLAUDE.md; `scripts/cal_mlp/` is also added to sys.path here for the bare `from integration import` calls later in the file). `from bot.constants import *` (Bit 3.1) at ~83; `from bot.helpers import *` + explicit underscore re-exports for `bot.helpers.validators` and `bot.helpers.breakers` (Bit 3.2) immediately after. `from bot.logger import Logger` (Bit 4.1) and `from bot.notifier import TelegramNotifier` (Bit 4.2) follow at ~100–~101. Verify with `grep -n "^from bot\." bot/_impl.py`. |
| ~125–~389 | Residual helpers + runtime-state singletons that stay in `bot/_impl.py` (`_swallow_persist_exception`, `_derive_subtype`/`_derive_asset_filter`/`_resolve_cal_engine` — deferred to Sprint 6 with `CalibrationEngine` because they read `_CAL_REGISTRY` module-level mutable state — `_append_raw_api_journal`, `_HPSB_VALIDATOR_UNAVAILABLE_REASON`, `_HPSB_MISSING_BLEEDERS = ...` / `_BLEED_BLOCK_MISSING_BLEEDERS = ...` boot-time invocations of the validators that themselves moved to `bot/helpers/validators.py`). |
| 390–26117 | Class definitions (see table below). One module-level `def discover_active_windows()` sits in the body region between SettlementTracker's class body and the MainLoop class def at line 24121; it ships with MainLoop in Bit 9.3. |

### Class-body end vs class-range note

The class table below uses *next-class-start − 1* as the range end. So
`SettlementTracker 22942–24120` includes the inter-class
...
SettlementTracker, and now over-counts KalshiClient by ~210 because the
inter-class space previously occupied by TelegramNotifier is now
unclaimed). Bit 4.2 deliberately did not insert a breadcrumb comment in
the cut site — `tests/test_notifier_extraction.py::test_telegram_notifier_class_not_defined_in_bot_impl`
enforces the negative contract.

## Classes (auto-verifiable)

Generated 2026-05-08 from `grep -nE '^class ' bot/_impl.py` (post-Bit-4.2 TelegramNotifier extraction; TelegramNotifier now lives in `bot/notifier.py`).

| Lines | Class | Size |
|---|---|---|
| 390–955 | `KalshiClient` | 566 |
| 956–3525 | `StateManager` | 2570 |
| 3526–3847 | `CoinbaseFeed` | 322 |
| 3848–3859 | `OrderbookSchemaError` | 12 |
| 3860–5629 | `KalshiFeed` | 1770 |
| 5630–5710 | `DeribitDVOLFetcher` | 81 |
| 5711–6038 | `CrossExchangeFeed` | 328 |
| 6039–6115 | `CoinGlassFetcher` | 77 |
| 6116–6237 | `OrderFlowEngine` | 122 |
| 6238–6398 | `KalshiOrderFlowTracker` | 161 |
| 6399–7358 | `VolatilityEngine` | 960 |
| 7359–7561 | `ProbabilityEngine` | 203 |
| 7562–8571 | `CalibrationEngine` | 1010 |
| 8572–17651 | `OpportunityScanner` | 9080 |
| 17652–22941 | `OrderExecutor` | 5290 |
| 22942–24120 | `SettlementTracker` | 1179 |
| 24121–26117 | `MainLoop` | 1997 |

## Project file map (root, 2026-05-05)

Live trading process:
- `bot/_impl.py` — main bot, all trading logic (26,117 lines post-Bit-4.2)
...
Tests: `tests/` (3,361 collected post-Bit-4.2; verify with `pytest tests/ --collect-only -q | tail -1`. Unit/integration/regression split pending Phase JJ)
...
- Sprint 4–6 → leaf classes (Bit 4.1 Logger SHIPPED 2026-05-08 → `bot/logger.py`; Bit 4.2 TelegramNotifier SHIPPED 2026-05-08 → `bot/notifier.py`; remaining: KalshiClient, fetchers, feeds, engines)
