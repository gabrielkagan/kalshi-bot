...
bot/_impl.py is **25,038 lines** as of 2026-05-08 (post-Bit-4.5a small-feeds extraction + writer-tracking instrumentation + BEGIN IMMEDIATE duration capture for db-locked RCA; Bit 3.1 moved ~1,200 lines of constants to `bot/constants.py`, Bit 3.2 moved ~790 lines of helpers to `bot/helpers/*`, Bit 4.1 moved 65 lines of Logger to `bot/logger.py`, Bit 4.2 moved 30 lines of TelegramNotifier to `bot/notifier.py`, Bit 4.3 moved 348 lines of KalshiClient to `bot/kalshi_client.py`, Bit 4.4 moved 141 lines of DeribitDVOLFetcher + CoinGlassFetcher to `bot/fetchers/*`, Bit 4.5a moved ~660 lines of CoinbaseFeed + OrderbookSchemaError + CrossExchangeFeed to `bot/feeds/*`). Class line ranges below
...
| 607–25038 | Class definitions (see table below). One module-level `def discover_active_windows()` sits in the body region between SettlementTracker's class body and the MainLoop class def; it ships with MainLoop in Bit 9.3. |
...
| Lines | Class | Size |
|---|---|---|
| 607–3235 | `StateManager` | 2629 |
| 3236–5025 | `KalshiFeed` | 1790 |
| 5026–5147 | `OrderFlowEngine` | 122 |
| 5148–5308 | `KalshiOrderFlowTracker` | 161 |
| 5309–6268 | `VolatilityEngine` | 960 |
| 6269–6471 | `ProbabilityEngine` | 203 |
| 6472–7481 | `CalibrationEngine` | 1010 |
| 7482–16572 | `OpportunityScanner` | 9091 |
| 16573–21862 | `OrderExecutor` | 5290 |
| 21863–23041 | `SettlementTracker` | 1179 |
| 23042–25038 | `MainLoop` | 1997 |

## Project file map (root, 2026-05-05)

Live trading process:
- `bot/_impl.py` — main bot, all trading logic (25,038 lines post-Bit-4.5a + writer-tracking + BEGIN IMMEDIATE duration capture)
