...
| 559–21128 | Class definitions (see table below). One module-level `def discover_active_windows()` sits in the body region between SettlementTracker's class body and the MainLoop class def; it ships with MainLoop in Bit 9.3. |

### Class-body end vs class-range note

The class table below uses *next-class-start − 1* as the range end. So
`SettlementTracker 17955–19133` includes the inter-class
...
`# ── Orphan-DB watchdog (Layer 3 of orphan prevention) ──`. Post-Bit-6.3
the `StateManager → OrderFlowEngine` gap and the
`KalshiOrderFlowTracker → OpportunityScanner` gap together contain
...
VolatilityEngine (Bit 6.1), ProbabilityEngine (Bit 6.2), and
CalibrationEngine (Bit 6.3) — all named with one-line `→ bot/<dest>`
breadcrumbs.
...
Generated 2026-05-10 from `grep -nE '^class ' bot/_impl.py` (post-Bit-6.3 CalibrationEngine extraction; CalibrationEngine now lives in `bot/engines/calibration.py` — third leaf in the `bot/engines/` subpackage and Sprint 6 closeout — alongside Bit-6.1 VolatilityEngine + Bit-6.2 ProbabilityEngine + Bit-4.5a/4.5b feeds + Bit-4.4 fetchers).

| Lines | Class | Size |
|---|---|---|
| 559–3247 | `StateManager` | 2689 |
| 3248–3369 | `OrderFlowEngine` | 122 |
| 3370–3573 | `KalshiOrderFlowTracker` | 204 |
| 3574–12664 | `OpportunityScanner` | 9091 |
| 12665–17954 | `OrderExecutor` | 5290 |
| 17955–19133 | `SettlementTracker` | 1179 |
| 19134–21128 | `MainLoop` | 1995 |

## Project file map (root, 2026-05-05)

Live trading process:
- `bot/_impl.py` — main bot, all trading logic (21,128 lines post-Bit-6.3 CalibrationEngine extraction + path-B singleton/helper relocation; Sprint 6 closes here)
