...
~219 lines over-count on `StateManager` from the pre-Bit-7.1 era is now
...
| Lines | Class | Size |
|---|---|---|
| 648–769 | `OrderFlowEngine` | 122 |
| 770–1001 | `KalshiOrderFlowTracker` | 232 |
| ~1010–~2179 | `SettlementTracker` | 1179 |
| ~2180–~4178 | `MainLoop` | 1999 |

(Post-Bit-9.1: OrderExecutor extracted to `bot/executor.py`; SettlementTracker
+ MainLoop line ranges shifted up by ~5,284 since the OrderExecutor body deleted.
Verify exact lines with `grep -nE '^class ' bot/_impl.py`.)

## Project file map (root, 2026-05-05)

Live trading process:
- `bot/_impl.py` — main bot, all trading logic (~4,179-line post-Bit-9.1 OrderExecutor extraction to `bot/executor.py` (path-A++ — `_append_raw_api_journal` relocated to `bot/helpers/raw_api_journal.py` via L81 alias-import; the `_get_order_executor()` helper retired from bot/scanner/__init__.py atomically with the `scanner-no-impl-toplevel` `.importlinter` contract drop, net contracts 6 → 5; the bot.executor ↔ bot.scanner cycle broken from the executor side via a `_get_opportunity_scanner()` method-body helper). Sprint 9 Bit 9.1 ships here.
...
- `bot/notifier.py` — `TelegramNotifier` class (fire-and-forget Telegram alerts) extracted from `bot/_impl.py` (Bit 4.2, ~50-line, stdlib + `requests` only — zero `bot.constants` deps, zero `bot.helpers` deps). Re-exported into `bot/_impl.py` via `from bot.notifier import TelegramNotifier` so the runtime construction in `MainLoop.__init__` (search `self.telegram = TelegramNotifier` for the current line) resolves. **Bit 8.1 path-A++ (2026-05-10)**: also hosts the module-level `_TELEGRAM: Optional["TelegramNotifier"] = None` singleton (relocated from `bot/_impl.py`). All three of `bot/_impl.py`, `bot/scanner/__init__.py`, and `bot/executor.py` (Bit 9.1, 2026-05-10) reach it via `import bot.notifier as _telegram_state` plus `_telegram_state._TELEGRAM` module-attribute access — the access pattern preserves mutation freshness across all three consumers (parallel to the Bit 6.3 path-B `_cal_state._CALIBRATION_ENGINE` pattern). `MainLoop.__init__` writes via `_telegram_state._TELEGRAM = self.telegram` (drops the previous `global _TELEGRAM` declaration). The `Optional["TelegramNotifier"]` forward-ref on the singleton type annotation lives at the def site here and has no in-tree `typing.get_type_hints` consumer.
