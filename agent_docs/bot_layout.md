...
bot/_impl.py is **25,052 lines** as of 2026-05-09 (post-Bit-4.5a small-feeds extraction + writer-tracking instrumentation + BEGIN IMMEDIATE duration capture + recent_writes ring buffer for FAST-fail intra-process culprit RCA; Bit 3.1 moved ~1,200 lines of constants to `bot/constants.py`, Bit 3.2 moved ~790 lines of helpers to `bot/helpers/*`, Bit 4.1 moved 65 lines of Logger to `bot/logger.py`, Bit 4.2 moved 30 lines of TelegramNotifier to `bot/notifier.py`, Bit 4.3 moved 348 lines of KalshiClient to `bot/kalshi_client.py`, Bit 4.4 moved 141 lines of DeribitDVOLFetcher + CoinGlassFetcher to `bot/fetchers/*`, Bit 4.5a moved ~660 lines of CoinbaseFeed + OrderbookSchemaError + CrossExchangeFeed to `bot/feeds/*`). Class line ranges below
...
| 607–25052 | Class definitions (see table below). One module-level `def discover_active_windows()` sits in the body region between SettlementTracker's class body and the MainLoop class def; it ships with MainLoop in Bit 9.3. |
...
| Lines | Class | Size |
|---|---|---|
| 607–3249 | `StateManager` | 2643 |
| 3250–5039 | `KalshiFeed` | 1790 |
| 5040–5161 | `OrderFlowEngine` | 122 |
| 5162–5322 | `KalshiOrderFlowTracker` | 161 |
| 5323–6282 | `VolatilityEngine` | 960 |
| 6283–6485 | `ProbabilityEngine` | 203 |
| 6486–7495 | `CalibrationEngine` | 1010 |
| 7496–16586 | `OpportunityScanner` | 9091 |
| 16587–21876 | `OrderExecutor` | 5290 |
| 21877–23055 | `SettlementTracker` | 1179 |
| 23056–25052 | `MainLoop` | 1997 |

## Project file map (root, 2026-05-05)

Live trading process:
- `bot/_impl.py` — main bot, all trading logic (25,052 lines post-Bit-4.5a + writer-tracking + BEGIN IMMEDIATE duration capture + recent_writes ring buffer)
