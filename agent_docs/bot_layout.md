...
2. **Static walk of locally-defined public classes inside `bot/_impl.py`** —
   post-Bit-7.1 the resident classes are `OrderFlowEngine`,
   `KalshiOrderFlowTracker`, `OpportunityScanner`, `OrderExecutor`,
   `SettlementTracker`, `MainLoop` (verify with the class table below).
   The list is auto-derived, not hardcoded, so extractions naturally
   move classes out of this layer and into Layer 1 (`StateManager` →
   Bit 7.1; `CalibrationEngine` → Bit 6.3; `ProbabilityEngine` →
   Bit 6.2; `VolatilityEngine` → Bit 6.1; etc.). Full class signatures.
...
Post-Bit-7.1 the `pre-OrderFlowEngine` gap (where the StateManager
class body used to live, now a one-line breadcrumb) and the
`KalshiOrderFlowTracker → OpportunityScanner` gap together contain
the breadcrumbs for every Sprint-4 + Sprint-6 + Sprint-7 leaf
...
ProbabilityEngine (Bit 6.2), CalibrationEngine (Bit 6.3), and
StateManager (Bit 7.1) — all named with one-line `→ bot/<dest>`
breadcrumbs.
