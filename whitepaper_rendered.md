...
| **Settled trades** | 3,125 (2,899W / 224L / 2 breakeven) |
| **Win rate** | 92.8\% |
| **Live P&L (cumulative)** | $745.18 |
| **Live P&L (Kelly-comparable headline; excludes 1-contract weather + kill-switched hourly)** | $746.75 |
...
|---|---|---|---|---|
| 15M main (Kelly-sized) | 1,369 | 1,240 W / 129 L | 651.85 | — |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0442 overall, 0.0315 on 15M, 0.3895 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0558 overall (2,942 samples), 0.0455 on 15M.

A small number of settled trades (3,125 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 687.69 | 1,303 | 1,197 W / — L | 0.0478 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | -65.93 | 355 | 314 W / — L | 0.0336 |
...
Counterfactual PnL for shadow-only strategies (would-have entered at relaxed gates), summed across all evaluated_opportunities with `counterfactual_pnl IS NOT NULL`: $-149,441.01 across 149,375 signals. These are simulated under the assumption of no fill impact, so they overstate what live promotion would actually capture; treat them as upper bounds when evaluating shadow→live promotions.
...
*Last updated: 2026-04-29T15:20:21Z*
