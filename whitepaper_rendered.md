...
| **Settled trades** | 4,278 (3,954W / 322L / 2 breakeven) |
| **Win rate** | 92.4\% |
...
|---|---|---|---|
| 15M main (Kelly-sized) | 1,710 | 1,545 W / 165 L | 90.4\% |
| Decided contracts | 319 | 308 W / 11 L | 96.6\% |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0407 overall, 0.0298 on 15M, 0.4024 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0605 overall (4,064 samples), 0.0449 on 15M.

A small number of settled trades (4,278 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 2,456 | 2,252 | 91.7\% | 0.0408 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | 1,508 | 1,369 | 90.8\% | 0.0336 |
...
*Last updated: 2026-05-16T17:35:12Z*
