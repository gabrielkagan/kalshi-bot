...
| **Settled trades** | 3,810 (3,528W / 280L / 2 breakeven) |
| **Win rate** | 92.6\% |
...
|---|---|---|---|
| 15M main (Kelly-sized) | 1,582 | 1,435 W / 147 L | 90.7\% |
| Decided contracts | 233 | 225 W / 8 L | 96.6\% |
| Weekend discount | 162 | 152 W / 10 L | 93.8\% |
| Overnight discount | 90 | 87 W / 3 L | 96.7\% |
| LPNE (BTC 80–87¢ near-expiry) | 5 | 5 W / 0 L | 100.0\% |
| Weather NO (1-contract verification) | 140 | 54 W / 86 L | 38.6\% |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0409 overall, 0.0292 on 15M, 0.3961 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0581 overall (3,612 samples), 0.0438 on 15M.

A small number of settled trades (3,810 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 1,988 | 1,826 | 91.9\% | 0.0412 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | 1,040 | 943 | 90.7\% | 0.0307 |
...
*Last updated: 2026-05-10T12:01:10Z*
