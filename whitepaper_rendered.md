...
| **Settled trades** | 3,800 (3,519W / 279L / 2 breakeven) |
| **Win rate** | 92.6\% |
...
|---|---|---|---|
| 15M main (Kelly-sized) | 1,581 | 1,434 W / 147 L | 90.7\% |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0411 overall, 0.0293 on 15M, 0.3988 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0582 overall (3,602 samples), 0.0439 on 15M.

A small number of settled trades (3,800 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 1,978 | 1,817 | 91.9\% | 0.0415 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | 1,030 | 934 | 90.7\% | 0.0311 |
...
*Last updated: 2026-05-10T00:38:52Z*
