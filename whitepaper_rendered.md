...
| **Settled trades** | 3,910 (3,616W / 292L / 2 breakeven) |
| **Win rate** | 92.5\% |
...
|---|---|---|---|
| 15M main (Kelly-sized) | 1,614 | 1,461 W / 153 L | 90.5\% |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0412 overall, 0.0299 on 15M, 0.3900 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0585 overall (3,712 samples), 0.0442 on 15M.

A small number of settled trades (3,910 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 2,088 | 1,914 | 91.7\% | 0.0416 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | 1,140 | 1,031 | 90.4\% | 0.0327 |
