...
| NO-side entry price range | 39–40¢ (39¢ floor since May 2; stepped 36→37 May 1, then 37→39 May 2) |
...
| **Settled trades** | 4,285 (3,961W / 322L / 2 breakeven) |
| **Win rate** | 92.4\% |
...
|---|---|---|---|
| 15M main (Kelly-sized) | 1,712 | 1,547 W / 165 L | 90.4\% |
| Decided contracts | 322 | 311 W / 11 L | 96.6\% |
| Weekend discount | 188 | 178 W / 10 L | 94.7\% |
| Overnight discount | 100 | 96 W / 4 L | 96.0\% |
| LPNE (BTC 80–87¢ near-expiry) | 5 | 5 W / 0 L | 100.0\% |
| Weather NO (1-contract verification) | 172 | 68 W / 104 L | 39.5\% |
...
- **Brier (all live candidates)** — measures the **model's** calibration on every opportunity that passed the live-candidate filter, whether or not it filled: 0.0406 overall, 0.0298 on 15M, 0.4024 on weather (side-aware: NO-side rows use $1-p_{raw}$ as the model's probability of the bot's bet winning).
- **Brier (filled trades only)** — measures the **bot's paid-decision** calibration via JOIN(settled_trades, latest matching evaluated_opportunities row), deduplicated on stacked tickers and timestamp ties: 0.0604 overall (4,070 samples), 0.0448 on 15M.

A small number of settled trades (4,285 total, of which N lack a matching EO row — see `settled_without_matching_eo` in the auto-generated stats) are excluded from filled-Brier; their model prediction was not preserved in evaluated_opportunities.
...
|---|---|---|---|---|
| Since 2026-04-11T20:43Z (loss-burst cooldown + weather NO live) | 2,463 | 2,259 | 91.7\% | 0.0407 |
| Since 2026-04-23T23:46Z (WS schema fix `0ddcaf8`) | 1,515 | 1,376 | 90.8\% | 0.0335 |
...
Daily high temperature markets across 19 US cities. Bracket and threshold contracts settling based on the observed daily high. Probability from 82-member NWP ensemble (GFS + ECMWF). NO-side has been LIVE since 2026-04-11 in 1-contract verification mode (entry zone 39–40¢, 39¢ floor since May 2 — stepped 36→37 May 1 then 37→39 May 2 as sub-bands underperformed, STC ≥ 16h before settlement); YES-side remains observation-only (research verdict: Gaussian fit materially overestimates YES probability vs. actual outcomes — which is why NO at 39–40¢ is the profitable side). Per-city CalEngines train on every settlement; bias correction tracks per-city forecast-vs-actual error.
...
| **Startup** | `ops/kalshi-bot.service` (source-of-truth in git, installed via `ops/install.sh`) → `start.sh` (venv + `.env`) → `python3 -m bot` |
...
- **Weather NO-side** — live in 1-contract verification mode (NO 39–40¢, STC≥16h)
...
*Last updated: 2026-05-16T19:01:30Z*
