...
def select_stale_snapshot(
    *,
    ...
    Pinned no-look-ahead invariant. Implementation must be strictly ≤; any
    post-event row indicates a look-ahead bug.
    """
    return [row for row in moc_rows if row["observation_time"] <= sigma_move_time]
