...
def _pipe(cmd):
    ...
    Ticket 86bbvrx1t. `pipefail` + explicit bash are required: without them
    the pipeline reports the LAST command's status, so a zstd dying
    mid-stream is masked by a healthy grep/xargs and the caller silently
    receives a prefix. The `exhausted` flag keeps a caller's early `break`
    (SIGPIPE, legitimately non-zero) from being read as a truncation.
    """
