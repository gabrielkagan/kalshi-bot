...
ASSETS = ("BTC", "ETH", "SOL", "XRP", "HYPE", "DOGE", "BNB", "ADA", "BCH", "NEAR", "ZEC")  # +NEAR/ZEC 2026-09-05 (86bbvdc8y; bronze has them since launch)
...
def _zst_lines(path: str):
    # ticket 86bbvrx1t: this line previously read
    #     subprocess.run([...], capture_output=True).stdout.decode()
    # which DISCARDS the exit code — on a truncated file zstd writes a PREFIX
    # to stdout and exits non-zero, and .stdout hands back the prefix silently.
    # This is the read path for ~38 of the algo_zoo mechanisms, so a short read
    # here quietly shrinks the corpus under every one of them. run_zstd_checked
    # raises instead (no `exhausted` subtlety: subprocess.run always reads to
    # completion, so any non-zero exit IS a truncation).
    raw = run_zstd_checked(path)
