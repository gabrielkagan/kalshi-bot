...
- `collector_health_monitor.py` (D1.6 + D2.5 + B3-fu3 + D1.8) — multi-tier cron-driven Telegram alert dispatcher: 5 checks on kalshi-collector (disk / ws_reconnects / collector_active / dropped_frames + `boot_state` post-86bbvdcat 2026-09-05) + 4 checks × 2 other WS-collector tiers (kalshi-coinbase-collector, kalshi-venue-l2-collector) + 3 checks × 2 HTTP-poll tiers (kalshi-weather-collector, kalshi-espn-collector) + 1 bot check (insert_evaluated_opportunity failures) = 20 dispatches per tick. Per-tier dedup-key prefixes `d1_6_*` / `d2_5_*` / `d1_8_*` / `d1_11_*` / `b3_fu3_*` (+ venue-L2 prefix). Lazy import of `bot.notifier.TelegramNotifier`; always exits 0 (cron convention). Operator-installed cron entry per the module docstring.
