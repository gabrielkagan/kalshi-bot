...
## Conventions
- **Regime-filter every analysis.** Identify when the relevant config changed (`git log market_config.py bot/_impl.py`) and filter `settled_trades` to that regime only. Pre-regime data is misleading.
