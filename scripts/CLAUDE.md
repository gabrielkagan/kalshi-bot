...
- `scripts/ops/` — operator-facing one-shots + setup + migrations + lock sentinels + Mac launchd jobs (56 files).
...
- **Cell-block activations deflate `filter_stage='candidate'` rollups.** Audit/dashboard scripts that filter `WHERE filter_stage = 'candidate'` for "all 15M trades" totals under-count post-activation. Bleed-cell stage VALUES to UNION: `'96C_SOL_XRP_STC_DANGER_BAND'`, `'TM98_97_98C_2_5MIN_BLEED'`, `'SOL_TAKER_85_89C_2_5MIN_BLEED'`, `'SOL_BLEED_V2_88_93C_2_5MIN'`, `'orderbook_prior_block'` (B1 Gate A, 86ba1zdwm), `'hype_high_price_buf_block'` (B1 Gate B, 86ba1zdwm). (Canonical 7-set including baseline `'candidate'` lives in `bot.helpers.cohort_attribution.COHORT_PARTITION_STAGES` — Money Printer Roadmap P1.1, ticket `86b9x3kgd`; extended by B1 to 7-set 2026-05-21.) Confirmed-affected list + cross-cutting detail: `bot/CLAUDE.md`. Decision: `kb/decisions/bleed-cell-blocks-2026-04-30.md`.
...
- `journal_archives_s3_sync.py` — incremental S3 sync of `~/kalshi-bot-repo/journal_archives/` via `rclone copy --checksum --immutable` (NOT `sync` — `sync` would mirror-delete S3 objects when local rotation prunes at 14d (`ROTATE_LOCAL_RETENTION_DAYS`); `copy` is one-way). Excludes live `*.jsonl` (current, uncompressed — ≤4 h span under the every-4h rotation) AND `rotation.log` (which `rotate_journals.sh` appends to every run — would trip `--immutable` otherwise). Idempotent: re-runs are no-ops via S3 ETag short-circuit. Invoked every 4h by `kalshi-journal-archives-sync.timer` (00/04/08/12/16/20:30 UTC, 30 min after each paired `ops/rotate_journals.sh` tick which itself runs every 4h on the hour post-2026-05-17; cadence revised from daily 04:30 UTC by ticket `86b9zkp89`). Ticket `86b9xgp7k`. Archive names are hour-stamped `<journal>_YYYY-MM-DDTHH.jsonl.zst` since 2026-09-05 (ticket `86bbvd50a`; the date-only stamp collided under the every-4h cadence and destroyed 16 of every 24 h of opportunity/scan journal for 109 days — `kb/failures/vps-disk-full-journal-rotation-collision-sep05.md`).
...
- `espn_live_probe.py` (86bbvqhyr, 2026-09-06) — one scoreboard GET with the shared `bot.engines.sports_engine.ESPN_USER_AGENT`; exit 0 on HTTP 200, 1 on any other status, 2 on transport error. `--sport/--league` pick the endpoint (default football/college-football), `--ua` swaps the UA for RCA. Run after every collector/bot deploy and whenever any of the three 86bbvqhyr alerts fires: `d1_11_http_errors`, `b3_fu3_espn_poll_errors`, `b3_fu3_sports_eval_silence`.
- `collector_health_monitor.py` (D1.6 + D2.5 + B3-fu3 + D1.8 + 86bbvqhyr) — multi-tier cron-driven Telegram alert dispatcher: 5 checks on kalshi-collector (disk / ws_reconnects / collector_active / dropped_frames + `boot_state` post-86bbvdcat 2026-09-05) + 4 checks × 2 other WS-collector tiers (kalshi-coinbase-collector, kalshi-venue-l2-collector) + 3 checks on kalshi-weather-collector + 4 on kalshi-espn-collector (+ `http_errors` post-86bbvqhyr 2026-09-06: per-league rolling-1h non-200 rate >50% from the sidecar's `espn_http_status_1h`) + 3 bot checks (insert_evaluated_opportunity failures + two from 86bbvqhyr: `sports_eval_silence` — the engine's `sports_health.json` sidecar shows ≥20 live-game ticks in 24 h with 0 sports rows in `evaluated_opportunities`, or the sidecar is missing/stale while the unit is active — and `espn_poll_errors`, >50% of leagues non-200 on the engine's last poll, which is the bot-side detector for the 403 class that `sports_eval_silence` structurally cannot see; no journal decode) = 23 dispatches per tick. Per-tier dedup-key prefixes `d1_6_*` / `d2_5_*` / `d1_8_*` / `d1_11_*` / `b3_fu3_*` (+ venue-L2 prefix). Lazy import of `bot.notifier.TelegramNotifier`; always exits 0 (cron convention). Operator-installed cron entry per the module docstring.
...
### Mac-side launchd jobs (operator's Mac, NOT the VPS)
- `discover_15m_series.py` + `discover_15m_series.sh` + `launchd/io.kalshi.15m-discovery.plist` (ticket `86bbvdc8y`, 2026-09-05) — daily 13:30-local Kalshi 15M-series discovery: public `/series` catalog (`fifteen_min`) ∪ latest `kalshi_rest/markets` bronze chunk → diff vs `~/kalshi-15m-discovery/known_series.json`, alert on NEW series (Telegram if the wrapper finds `TELEGRAM_*` in `.env`, else macOS notification; always a sentinel file), first-seen from public REST paging (skipped on the silent bootstrap run), lists the 15M series the bot does not trade. Loud failure path: `LAST_FAILURE.txt` + `DISCOVERY_FAILED_*` sentinel + exit 1. Install per the plist header comment. Motivation: NEAR/ZEC/commodity 15M ran 2+ months unnoticed (see `collector/rest_snapshot.py` GENERALIZED note).
