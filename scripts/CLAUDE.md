...
- `collector_health_monitor.py` (D1.6 + D2.5 + B3-fu3 + D1.8 + 86bbvqhyr) — multi-tier cron-driven Telegram alert dispatcher: 5 checks on kalshi-collector (disk / ws_reconnects / collector_active / dropped_frames + `boot_state` post-86bbvdcat 2026-09-05) + 4 checks × 2 other WS-collector tiers (kalshi-coinbase-collector, kalshi-venue-l2-collector) + 3 checks on kalshi-weather-collector + 4 on kalshi-espn-collector (+ `http_errors` post-86bbvqhyr 2026-09-06: per-league rolling-1h non-200 rate >50% from the sidecar's `espn_http_status_1h`) + 2 bot checks (insert_evaluated_opportunity failures + `sports_eval_silence` post-86bbvqhyr: the engine's `sports_health.json` sidecar shows ≥20 live-game ticks in 24 h, 0 sports rows in `evaluated_opportunities`; no journal decode) = 22 dispatches per tick. Per-tier dedup-key prefixes `d1_6_*` / `d2_5_*` / `d1_8_*` / `d1_11_*` / `b3_fu3_*` (+ venue-L2 prefix). Lazy import of `bot.notifier.TelegramNotifier`; always exits 0 (cron convention). Operator-installed cron entry per the module docstring.
