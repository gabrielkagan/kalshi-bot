...
- `export_market_obs_to_s3.py` — nightly archive of `market_observations_continuous` (the only retention-pruned table) to S3 via Parquet+zstd. Read-only SQLite connection; target_date = today - 4d (day 4 of the 5d retention window; tightened from 13d/14d to 4d/5d by ticket 86ba0jb39 2026-05-19 to reduce executemany lock-hold tail — lockstep pinned by `tests/contracts/test_market_obs_retention_lockstep.py`). Invoked nightly by `kalshi-market-obs-archive.timer` (05:30 UTC). Ticket `86b9xcdwg`.
