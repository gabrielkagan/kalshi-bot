...
- **`busy_timeout` enforcement**: `grep` every `sqlite3.connect()` call across all `.py` files. Assert each one includes `PRAGMA busy_timeout` within the next 5 lines. Catches the bot/engines/sports_engine.py "database is locked" class of bug.
