...
- `MemoryHigh=400M` + `MemoryMax=512M` + `MemorySwapMax=0` — soft cgroup throttle at 400M (78% of MemoryMax) added 2026-05-20 (umbrella `86ba12rf0`) after the 2026-05-19 unexplained collector exits at 22:06 / 22:26 / 00:04 UTC correlated with peak memory at 472M / 512M cap (92%). `MemoryHigh` triggers kernel-side throttling at the soft cap, surfacing memory pressure in cgroup pressure counters BEFORE the hard SIGKILL at `MemoryMax`. The 0 swap is the failure mode (D0.2 §3.2 measured 47 MB/conn × 7-8 conns = ~376 MB; 512 MB cap leaves writer/uploader headroom).
