...
- `MemoryMax=384M` + `MemorySwapMax=0` — bumped 256M → 384M on 2026-05-30 when ADA-USD + BCH-USD were added (7 → 9 products); unchanged when NEAR-USD + ZEC-USD joined on 2026-09-05 for ticket `86bbvdc8y` (9 → 11 products via `collector/coinbase_main_loop.COLLECTOR_PRODUCT_IDS`; the bot still trades 7 and the shared `coinbase_wire.DEFAULT_PRODUCT_IDS` stays at 7). Sizing history: measured 7-product peak 229 MiB/256 MiB (90%); measured 9-product MemoryPeak 229 MiB (240,500,736 B)/384 MiB on 2026-09-05 — flat vs 7 products, so +2 products keeps ≥150 MiB headroom. Coinbase single-conn × 11 products × 5 channels (post-D2.5 level2_batch promotion) is structurally lighter than Kalshi 7-conn × ~21K-subs (host has 5.4G free — the per-unit cap is the only constraint). The unit file's own comment still says 9 products — left untouched on purpose (deploy.yml unit-drift guard demands `install.sh` on any unit edit); refresh it with the next unit change (ticket `86bbvfpmr`). Leaves room for worker queue (10K items) + zstd buffer + rclone overhead.
...
`sudo systemctl stop kalshi-espn-collector` → bot + Kalshi/Coinbase/Weather collectors all unaffected. `scripts/ops/collector_health_monitor.py` extends to FIVE-TIER dispatch (SIX post-B2a-1 venue-L2) with dedup prefix `d1_11_*`; same HTTP-poll subset as weather (disk + collector_active + dropped_frames; NO ws_reconnects) **plus, post-86bbvqhyr (2026-09-06), `http_errors`** — `check_espn_http_errors` reads the per-league rolling-1h `espn_http_status_1h` block that `collector/espn_main_loop.py::write_bronze_health_sidecar` publishes (schema_version stays 1, additive) and alerts at >50% non-200 for any league with ≥10 polls in the window (dedup `d1_11_http_errors`). The bot tier gained `sports_eval_silence` in the same Bit (`check_sports_eval_silence`: the sports engine's own `sports_health.json` sidecar (written next to `state.db` every 30 s tick by `SportsEngine._note_tick`) reports ≥20 ticks with live ESPN games in its 24 h window AND 0 `product_type='sports'` rows in `evaluated_opportunities` for that window; it ALSO alerts `*BOT SPORTS ENGINE SILENT*` when that sidecar is absent and `*BOT SPORTS ENGINE WEDGED*` when it is older than 900 s while `kalshi-bot` is active past its 900 s boot grace — the sports engine is a daemon thread that `bot/main_loop.py` sets to `None` and continues without if it fails to start, nothing calls `is_alive()` on it, and no other check polls the bot unit, so a dead sports thread inside a healthy bot was invisible to every check (R6-MAJOR-1) — no journal decode: a 24 h `journalctl -g` pull measured ~50 s on the VPS, so the engine self-reports (24 h, not shorter: healthy days have 1-43 sports rows clustered 17:00-05:00 UTC, so a 3 h window false-alarms several times a day); read-only sqlite at `STATE_DB_PATH` else `~/kalshi-bot-repo/state.db`; dedup `b3_fu3_sports_eval_silence`). A THIRD check, `check_bot_espn_poll_errors` (dedup `b3_fu3_espn_poll_errors`), is the bot-side detector for the 403 class itself: >50% of leagues non-200 or raised on the engine's last poll, read from the same `sports_health.json`. It exists because `sports_eval_silence` structurally cannot see a 403 — the poll raises, `poll_all_leagues` returns `{}`, `live_ticks_in_window` stays 0, and its ≥20-live-tick precondition never arms (found at R6 of the gate). All three exist because ESPN 403'd the `KalshiBot/1.0` User-Agent for ~5 weeks while disk / collector_active / dropped_frames stayed green — "chunks landing" ≠ "data landing" (`kb/failures/espn-403-user-agent-silent-outage-sep06.md`). **deploy.yml gained a path-aware `kalshi-espn-collector` restart block in the same Bit** (R4 of the gate found the ESPN collector had NO restart block, so a push touching `collector/espn_archiver.py` would have left the old process — old UA, no `espn_http_status_1h` key — running while the deploy looked green; mirror of the D2.5 Coinbase block, path set `collector/espn_archiver.py` / `collector/espn_main_loop.py` / `collector/writer.py` / `collector/uploader.py` / `kalshi_wire/ws_client.py` / `ops/kalshi-espn-collector.service` / `espn-collector-start.sh` / `requirements.txt`, unit-drift guard, `sudo -n`; pinned by `tests/contracts/test_deploy_yml_path_aware_espn_collector_restart.py`; `kalshi-weather-collector` still has no block — ticket `86bbvr4vk`, which also carries a structural ask: one contract test asserting every `ops/kalshi-*.service` has a deploy.yml restart block, with venue-L2 allow-listed as deliberately manual). Post-deploy verification, in this order: (1) `python3 scripts/ops/espn_live_probe.py` on the VPS (exit 0 on 200, 1 on other status, 2 on transport error); (2) `espn_http_status_1h` present in `/var/lib/kalshi-espn-collector/bronze_health.json` with `non_200 == 0`; (3) `http_status: 200` in the next bronze chunk; (4) `sports_health.json` appears in `~/kalshi-bot-repo/` within one 30 s tick. Sports bronze restarts at the first 200 chunk after deploy — ESPN keeps no scoreboard history.

### LEAGUES_ESPN drift discipline

`collector/espn_archiver.py` defines `LEAGUES_ESPN` (**23 entries as of 2026-09-06**; 24 at D1.11.a ship) mirroring `bot.engines.sports_data.LEAGUES` enabled+espn-eligible subset. **Why 23:** ticket `86bbvt12r` detached `fifa.worldcup` on 2026-09-06 — ESPN returns HTTP 400 for that league upstream (reproduced with the shipped UA while `usa.1` and `college-football` returned 200 in the same loop, so NOT a User-Agent issue; the 5-week blanket 403 had masked it). At 100% non-200 it fired `d1_11_http_errors` every 5 minutes indefinitely — alert fatigue is what let the original outage run unnoticed, so the league was detached at source rather than the check being silenced. Mechanism is `espn_sport=espn_league=None` on `KXWCGAME` (the existing pattern for a series with no usable ESPN endpoint, as with the esports entries and `KXAFCONGAME`), NOT `enabled=False` — that is reserved for the circuit breaker by `test_step3_soccer_reenabled`, and the Kalshi series itself is healthy. Kalshi price monitoring for `KXWCGAME` continues. **Do not 'correct' 23 back to 24** — historical references to 24 (the D1.11.a ship count and the 5-week 403 outage) are accurate and must stay. To re-attach, restore BOTH sides and restart the unit. Contract test `tests/contracts/test_collector_espn_archiver.py::test_leagues_espn_mirrors_bot_leagues` fails RED on drift — operator must update both sides + `sudo -n /bin/systemctl restart kalshi-espn-collector` so the new channel registers as a BronzeWriter on next boot.
...
### No path-aware deploy block (operator restart required on code change)

Unlike `kalshi-collector` and `kalshi-coinbase-collector`, `.github/workflows/deploy.yml` has NO restart block for `kalshi-venue-l2-collector` (re-verified 2026-09-06 — still no `venue-l2` match anywhere in deploy.yml). A push that changes `collector/venue_l2_archiver.py` matches the kalshi-collector `^collector/` path regex and restarts THAT unit only — the venue-L2 unit keeps running the old code until the operator runs `sudo systemctl restart kalshi-venue-l2-collector` (NOPASSWD line above). Surfaced by ticket `86bbvdc8y` adv-review R4 when NEAR/ZEC were added to `VENUE_SYMBOLS`; verify a symbol-map change took effect with `journalctl -u kalshi-venue-l2-collector -n 200 | grep -Ei "NEAR|ZEC"` and fresh `kraken_ws/book` chunks in S3 after the restart. A venue-L2 deploy block is a filed follow-up (see the 86bbvdc8y ticket comments).
