...
### S3 bucket policy + lifecycle + IAM (F6 from D0.1 — RESOLVED at D1.5)

Before the first bronze chunk lands, four S3-side surfaces must be at the canonical 6-prefix shape (`daily/`, `journals/`, `market_obs/`, `bronze/`, `silver/`, `gold/`):

1. **Lifecycle policy** (§2 of `scripts/STATE_DB_BACKUP_SETUP.md`) — bronze/silver rules added (Standard → DEEP_ARCHIVE @ 30d for bronze; Standard → GLACIER_IR @ 90d for silver; gold no-op default).
2. **Bucket policy Deny** (§2b) — `s3:DeleteObjectVersion` deny statement Resource array expanded from `daily/*` only to all 6 prefixes (ransomware protection against compromised collector creds).
3. **Writer IAM** (§3) — `s3:PutObject` Resource expanded from `daily/*` only to all 6 prefixes. **Without this, the collector's first `rclone copyto` returns 403, the KEEP-local-on-failure posture accumulates chunks in `outbox/`, and the 2 GB VPS fills the root volume within hours of bronze day-zero.**
4. **Reader IAM** (§5) — `s3:GetObject` / `s3:GetObjectVersion` / `s3:RestoreObject` Resource lists + `s3:ListBucket` prefix Condition expanded similarly, preemptively unblocking the D2.x off-VPS DuckDB+dbt silver/gold ETL chain.

On a fresh bucket: run §1-§5 verbatim from the runbook (templates are now at the D1.5 6-prefix shape). On an existing pre-D1.5 bucket: §1 (CreateBucket) is a no-op + §2 (lifecycle) requires `GET-merge-PUT` to preserve the audited daily/ cadence drift (see `scripts/STATE_DB_BACKUP_SETUP.md` §2 Operator note); §2b/§3/§5 (bucket-policy + writer-IAM + reader-IAM) IAM `put-*-policy` calls ARE idempotent and safe to re-run verbatim. See §12 "Bucket-side multi-prefix expansion" for the verification one-liners that catch a half-extended bucket BEFORE the collector starts.
