...
`sudo systemctl stop kalshi-espn-collector` → bot + Kalshi/Coinbase/Weather collectors all unaffected. `scripts/ops/collector_health_monitor.py` extends to FIVE-TIER dispatch (SIX post-B2a-1 venue-L2) with dedup prefix `d1_11_*`; same HTTP-poll subset as weather (disk + collector_active + dropped_frames; NO ws_reconnects) **plus, post-86bbvqhyr (2026-09-06), `http_errors`** — `check_espn_http_errors` reads the per-league rolling-1h `espn_http_status_1h` block that `collector/espn_main_loop.py::write_bronze_health_sidecar` publishes (schema_version stays 1, additive) and alerts at >50% non-200 for any league with ≥10 polls in the window (dedup `d1_11_http_errors`). The bot tier gained `sports_eval_silence` in the same Bit (`check_sports_eval_silence`: the sports engine's own `sports_health.json` sidecar (written next to `state.db` every 30 s tick by `SportsEngine._note_tick`) reports ≥20 ticks with live ESPN games in its 24 h window AND 0 `product_type='sports'` rows in `evaluated_opportunities` for that window — no journal decode: a 24 h `journalctl -g` pull measured ~50 s on the VPS, so the engine self-reports (24 h, not shorter: healthy days have 1-43 sports rows clustered 17:00-05:00 UTC, so a 3 h window false-alarms several times a day); read-only sqlite at `STATE_DB_PATH` else `~/kalshi-bot-repo/state.db`; dedup `b3_fu3_sports_eval_silence`). Both exist because ESPN 403'd the `KalshiBot/1.0` User-Agent for ~5 weeks while disk / collector_active / dropped_frames stayed green — "chunks landing" ≠ "data landing" (`kb/failures/espn-403-user-agent-silent-outage-sep06.md`). Post-deploy canary: `python3 scripts/ops/espn_live_probe.py` (exit 0 on 200, 1 on other status, 2 on transport error). Sports bronze restarts at deploy — ESPN keeps no scoreboard history.
