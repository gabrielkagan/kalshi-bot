...
| Narrow `OperationalError` catch missed bare-`DatabaseError` stale-cursor raises | **New rule (Bit `bit-state-py-database-error-catch`, 2026-05-22):** sqlite3 exception catches at retry-on-busy sites must use the broadest base class compatible with the dispatch logic (typically `sqlite3.DatabaseError`); message-text string-match inside the handler does the fine-grained routing. Past-48h journal histogram showed 23 `DatabaseError` events escaping the narrow catch at the StateManager BEGIN IMMEDIATE retry loops. See `kb/failures/database-contention.md` Incident 6 for full incident detail + pinned AST guard. |
